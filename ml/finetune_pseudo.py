"""Self-training of the base model on our own pseudo-labels (legally
clean: teacher = our ensemble, labels are ours; real labels come from
GuitarSet CC-BY and McGill Billboard CC0).

Train = pseudo windows (teacher-confidence masked) + real guitar windows
(clean + consumer-mic aug) + real Billboard windows; pitch-shift
augmentation on everything — the student sees modern mixes transposed,
which the teacher never did. Checkpoint selection uses ONLY the clean
sets (mean of guitar_test_clean and billboard_test); hook_val (HookTheory,
CC BY-NC-SA) is printed as a monitor and never influences selection.

Self-contained for a rented GPU box: torch + numpy + vendor/BTC-ISMIR19.

Run (GPU or CPU):
    python finetune_pseudo.py [--epochs 40] [--batch 32] [--smoke]
"""
import argparse
import os
import sys
...
def main():
    ...
    parts = {name: load_pack(name + ".npz")
             for name in ("pseudo_train", "guitar_train", "billboard_train")}
    ...
    tests = {name: load_pack(name + ".npz")
             for name in ("guitar_test_clean", "billboard_test", "hook_val")}
    select_on = ("guitar_test_clean", "billboard_test")   # clean data only
    ...
    def selection_score(scores):
        return sum(scores[n] for n in select_on) / len(select_on)

    scores = evaluate_all()
    print("baseline: " + "  ".join(f"{k} {v:.4f}" for k, v in scores.items()), flush=True)
    ...
    print(f"best clean-selection score: {best:.4f}", flush=True)
