"""Robustness fine-tune of BTC (large voca) for the guitar model, v2.

Recipe vs v1 (finetune.py): consumer-mic augmented GuitarSet windows in
training AND in a second held-out metric, plus a Billboard subsample so
the model keeps its general (non-diatonic, full-mix) harmony instead of
overfitting to GuitarSet's studio/diatonic niche.

Self-contained for a rented GPU box: torch + numpy, vendor/BTC-ISMIR19
(code + base checkpoint) and data/ window packs next to this script.
Checkpoint selection: best mean of clean & augmented guitar held-out
majmin; Billboard majmin is printed as a forgetting monitor.

Run (GPU or CPU):
    python finetune_guitar2.py [--epochs 40] [--batch 32] [--smoke]
"""
import argparse
import os
import sys
...
def main():
    ...
    parser.add_argument("--smoke", action="store_true",
                        help="tiny data slice, 1 epoch — script validation only")
    args = parser.parse_args()

    device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
    print(f"device: {device}", flush=True)

    gx, gy, gm = load_pack("guitar_train.npz")
    bx_all, by_all, bm_all = load_pack("billboard_train.npz")
    train_x = np.concatenate([gx, bx_all])
    train_y = np.concatenate([gy, by_all])
    train_m = np.concatenate([gm, bm_all])
    tests = {name: load_pack(name + ".npz")
             for name in ("guitar_test_clean", "guitar_test_aug", "billboard_test")}
    ...
    print(f"train windows: {len(train_x)} "
          f"(guitar {len(gx)}, billboard {len(bx_all)})", flush=True)
    ...
    best = (scores["guitar_test_clean"] + scores["guitar_test_aug"]) / 2
    since_best = 0
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        scores = evaluate_all()
        target = (scores["guitar_test_clean"] + scores["guitar_test_aug"]) / 2
        marker = ""
        if target > best:
            best = target
            since_best = 0
            torch.save({"model": model.state_dict(), "mean": mean, "std": std},
                       os.path.join(HERE, "btc_guitar2.pt"))
            marker = "  <-- saved"
        else:
            since_best += 1
    ...
    print(f"best guitar clean+aug mean: {best:.4f}", flush=True)
